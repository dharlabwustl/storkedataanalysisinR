...
# to_wide_format_one_variable<-function(all_combined_data_frames){
#   all_combined_data_frames11<-all_combined_data_frames %>% dplyr::select(subj_session_count,subject_id1,CSF.RATIO)
#   all_combined_data_frames11<-all_combined_data_frames11 %>% pivot_wider(
#          names_from = subj_session_count,
#          names_sep = ".",
#          values_from = CSF.RATIO,id_cols=subject_id1,names_prefix='SCAN_NUM'
#      )
...
all_combined_data_frames<-read_csv('all_combined_data_frames_04_16_2024.csv')
all_combined_data_frames <- all_combined_data_frames %>%
  mutate(ICH.PHE.RATIO = ICH.VOLUME/ICH.EDEMA.VOLUME)
numeric_colname='CSF.RATIO'
longitudinal_analysis(all_combined_data_frames,numeric_colname)
numeric_colname='NWU'
longitudinal_analysis(all_combined_data_frames,numeric_colname)
numeric_colname='delta_csf'
longitudinal_analysis(all_combined_data_frames,numeric_colname)
all_combined_data_frames11<-to_wide_format_one_variable(all_combined_data_frames)
...
#     measurement = fct_inorder(paste("Measure at scan", age))
#   )
column_to_measure<-'CSF.RATIO'
dental_long<-all_combined_data_frames_subset %>%dplyr::rename('id'='subject_id1','sex'='project_type','age'='timeinterval','distance'='CSF.RATIO','measurement'='timeinterval_asfactor') %>%
  set_variable_labels(
    age = "Age of the child at measurement",
    measurement = "Label for time measurement",
    distance = "Measurement"
